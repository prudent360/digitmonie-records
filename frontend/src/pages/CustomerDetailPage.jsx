    ...
    <div>
      ...
      <div className="bg-surface border border-border rounded-lg p-7 mb-7 max-md:p-5 max-sm:p-4">
        <div className="flex items-center gap-5 mb-7 pb-6 border-b border-border max-md:flex-col max-md:items-start max-md:gap-4">
          ...
          <div><h2 className="text-xl font-semibold text-text-primary">{customer.full_name}</h2><p className="text-text-muted text-sm">Added {new Date(customer.created_at).toLocaleDateString()}</p></div>
        </div>
        <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-7 max-md:gap-5 max-sm:grid-cols-2 max-sm:gap-4">
          <div className="flex flex-col gap-1.5"><span className="text-[11px] font-medium text-text-muted uppercase tracking-wider">Phone</span><span className="text-base font-semibold text-text-primary">{customer.phone}</span></div>
          <div className="flex flex-col gap-1.5"><span className="text-[11px] font-medium text-text-muted uppercase tracking-wider">Email</span><span className="text-base font-semibold text-text-primary">{customer.email || '—'}</span></div>
          <div className="flex flex-col gap-1.5"><span className="text-[11px] font-medium text-text-muted uppercase tracking-wider">Address</span><span className="text-base font-semibold text-text-primary">{customer.address || '—'}</span></div>
          <div className="flex flex-col gap-1.5"><span className="text-[11px] font-medium text-text-muted uppercase tracking-wider">ID Number</span><span className="text-base font-semibold text-text-primary">{customer.id_number || '—'}</span></div>
        </div>
      </div>

      <div className="bg-surface border border-border rounded-lg p-7 max-md:p-5 max-sm:p-4">
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-border"><h3 className="text-base font-semibold text-text-primary">Loans ({customer.loans?.length || 0})</h3></div>
        ...
        ) : <div className="flex flex-col items-center justify-center py-20 px-6 text-center bg-surface rounded-lg border border-dashed border-border"><p className="text-sm text-text-muted">No loans yet</p>{user.role !== 'viewer' && <Link to={`/loans/new?customer_id=${customer.id}`} className={`${btnPrimary} mt-3`}>Create Loan</Link>}</div>}
      </div>
    </div>
  );
}
